...
# add a description to the distance metric [Matthias]

# add a small text to the standardizsation of the data [Stefan]

# change order of insights in the data and the main metrics 

# "conspicuousness" --> new name and maybe the graph after the image of the map

# remove locations without map below [Stefan]

# text for the boxplot of latitude and longitude [Stefan]

# where should be the data preparation? maybe more up at the insights [Matthias]

### baseline as combined model as well? [Matthias]

# force anova output just on one page [Matthias]
...
# check the spelling of crossvalidation, cross validation, cross-validation, should be just one version
...
```{r insights_draw_worldmap, include=TRUE, echo=FALSE}
...
In Zhou et al. [@Zhou2015] they list two reasons as to why it's preferably not treated as a classification problem: One, the ratio between the large number of countries/areas and the number of examples per country/area is very disproportional and would result in poor classification results. And secondly, with regression we already have a natural error metric:
The geographical distance from the true position.
...
pred <- cbind(pred1, pred2)
min(distances(pred, data[test, c("Latitude","Longitude")]))
mean(distances(pred, data[test, c("Latitude","Longitude")])) ## final result in km
```

There is also a multivariate ridge regression model calculated, hence a single model with two outputs for latitude and longitude. The lambda is found by cross validation. This process was done in a separate file as it took more than an hour to get a good value for 

```{r ridge_multivariate_model, include=TRUE, echo=FALSE}
ridge.mod.multivar <- glmnet(x[train,], y[train,], family = "mgaussian", alpha=0, lambda=0.070)
pred.multivar <- predict(ridge.mod.multivar, s = 0.07, newx=x[test,])
mean(distances(pred.multivar, data[test, c("Latitude","Longitude")])) ## final result in km

#coef(ridge.mod.multivar)

ridge.mod.mv.thresh <- glmnet(x[train,], y[train,], family = "mgaussian", alpha=0, lambda=0.70, thresh = 0.03)
pred.mv.thresh <- predict(ridge.mod.mv.thresh, s = 0.7, newx=x[test,])
mean(distances(pred.mv.thresh, data[test, c("Latitude","Longitude")])) ## final result in km

#coef(ridge.mod.mv.thresh)

```



```{r matthias_playground, include=TRUE}
```
...
To use NNs for our predictions we will, as with all machine learning models, fit the data to the model. This is done by minimising the loss function, which for NN-regression is the mean of squared errors MSE over the training set.
...
The NN takes all features from the training data and outputs two values for the Latitude and Longitude, so it is a multi-output regression.
Inbetween Input and Output Layer are a few more layers defined which all use ReLU as activation function. For compiling and fitting the model we again use the Mean Squared Error (MSE) and Adam as the optimizer. We let the model train for 100 epochs with a batch size of 128 and validation split of 0.2.
