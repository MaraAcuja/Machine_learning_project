...
```{r linear_regression_boxplot, echo=FALSE, fig.cap="Comparison of error of baseline models"}
# TODO:  in GGPLOT2
...
```{r ridge_data_cv, include=TRUE, echo=FALSE, fig.cap="Cross-validation of separated ridge regression models"}
...
```{r show_cv, include=TRUE, echo=FALSE, fig.cap="Cross-validation for combined ridge regression model"}
...
The world map above shows a sample of the predicted and true values. The predicted values for the neural net (red) and the ridge regression (blue) are displayed together with the true value. The predicted values are also linked to the true value. This makes it easier to investigate the relationships. It is quite interesting to see that the predictions in the near and middle east are much better than for South America, Africa or Australia. This might be caused to the unbalanced data set where fewer data points from these last mentioned continents were available.
There is another quite interesting fact to see. The predictions were both wrong into the wrong direction, hence the angle between the connection lines is always quite small. This led us to investigate this behaviour further:
